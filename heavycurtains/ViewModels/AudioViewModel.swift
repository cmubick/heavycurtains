...
//  heavycurtains
//
//  Created by Chris Ubick on 11/24/23.
//
...
class AudioViewModel : NSObject, ObservableObject , AVAudioPlayerDelegate{
    ...
    func startRecording() {
        ...
        let fileName = path.appendingPathComponent("heavy-curtains: \(dateFormatter.string(from: date)).m4a")
    ...
    func updateFileName(url: URL, newName: String){
        let path = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        let fileName = path.appendingPathComponent("\(newName).m4a")
        try! FileManager.default.moveItem(at: url, to: fileName)
        for i in 0..<recordingsList.count {
            if recordingsList[i].fileURL == url {
                recordingsList[i].name = ("\(newName).m4a")
                recordingsList[i].fileURL = fileName.absoluteURL
            }
        }
    ...
    func fetchAllRecording(){
        ...
        for i in directoryContents {
            recordingsList.append(Recording(fileURL : i, createdAt:getFileDate(for: i), name: i.lastPathComponent, isPlaying: false))
        }
